#Problem 4
#Her bir while döngüsünde kullanıcıdan
#bir sayı alın ve kullanıcının girdiği
#sayıları "toplam" isimli bir değişkene ekleyin.
#Kullanıcı "q" tuşuna bastığı zaman döngüyü sonlandırın
#ve ekrana "toplam değişkenini" bastırın.

#İpucu : while döngüsünü sonsuz koşulla
#başlatın ve kullanıcı q'ya basarsa döngüyü break ile sonlandırın.
print("çıkmak için 'q' ya basın...")
toplam = 0
while(True):
    sayi = input("bir sayı girin: ")
    if(sayi == "q"):
        break
    toplam += int(sayi)
    print("toplam sayı: ",toplam)
print("kapatılıyor...")